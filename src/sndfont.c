...
//
static int
getoriginalpitch (int key, int pitch) {
    return ((pitch / 100) - key) * -1;
}

static double
attentopercent (int attenuation) {
...
void
SF_AddPreset (sfpheader_t * preset, char *name, int idx, int bank,
              int presetidx) {
...
void
SF_AddInstrument (sfinstheader_t * inst, char *name, int idx) {
...
static int npgens = 0;
void
SF_AddPresetGenerator (sfpgenheader_t * gen, generatorops_e op,
                       gentypes_t value) {
    sfpresetgen_t *pgen;
...
static int nibags = 0;
void
SF_AddInstrumentBag (sfibagheader_t * bag) {
    sfinstbag_t *instbag;
...
static int npbags = 0;
void
SF_AddPresetBag (sfpbagheader_t * bag) {
    ...
    for (i = 0; i < npatch; i++) {
        ...
        for (j = 0; j < p->length; j++) {
            ...
            if (sp->decaytime > 1) {
                ...
                SF_AddInstrumentGenerator (&inst->gen, GEN_VOLENVRELEASE,
                                           value);
            }
