...
void
Wad_Decompress (byte * input, byte * output) {
    int getidbyte = 0;
    int len;
    int pos;
    int i;
...
int
Wad_GetLumpNum (const char *name) {
    int i;
...
void *
Wad_GetLump (char *name, bool dcmpType) {
    byte *data;
...
void
Wad_AddOutputLump (const char *name, int size, cache data) {
    int padSize = 0;
    wadheader_t *header;
    lump_t *lump;
...
void
Wad_AddOutputSprite (d64ExSpriteLump_t * sprite) {
#ifdef USE_PNG
    int j = 0;
...
void
Wad_AddOutputTexture (d64ExTexture_t * tex) {
#ifdef USE_PNG
    int j = 0;
    int i = 0;
    int pos = 0;
...
void
Wad_AddOutputGfx (gfxEx_t * gfx) {
#ifdef USE_PNG
    png = Png_Create (gfx->width,
                      gfx->height,
                      16, gfx->palette, 8, gfx->data, gfx->lumpRef, &pngsize);
...
void
Wad_AddOutputHudSprite (d64ExSpriteLump_t * sprite) {
#ifdef USE_PNG
...
void
Wad_AddOutputPalette (d64PaletteLump_t * palette) {
    cache data;
    int size;
    char name[8];
    int pos = 0;
...
void
Wad_AddOutputMidi (midiheader_t * mthd, int index) {
    cache data;
    int i;
    int ntracks;
    int pos = 0;
