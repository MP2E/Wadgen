...
void
Rom_SwapBigEndian (int swaptype) {
    uint len;
    ...
    if (swaptype == 3) {
        ...
        for (swap = (int *) RomFile.data, len = 0; len < RomFile.length / 4;
             len++)
    ...
    // n64
        ...
        for (swap = (short *) RomFile.data, len = 0; len < RomFile.length / 2;
             len++)
...
bool
Rom_VerifyRomCode (const romLumpSpecial_t * l) {
